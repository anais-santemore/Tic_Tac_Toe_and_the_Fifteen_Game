...
function getPositionsArray() {
    let positionsArray = Array(10).fill([])
    positionsArray.push([[]])
    for (let i = 1; i <= 9; i++) {
        let parents = positionsArray[[i - 1]]
        console.log(`parents: ${parents}`);

        let children = []

        parents.forEach(parent => children.push(getChildren(parent)))
        console.log(`children: ${children}`);

        // positionsArray[i] = children.flat(1)
        positionsArray[i] = children
        console.log(`positionsArray index ${i}: ${positionsArray[i]}`);
...
// An array where each index corresponds to a possible length of moveList
console.log(`positionsArray: ${getPositionsArray()}`);
...
let positionMap = generatePositionMap()
